...
def extract_information(pages_text):
    ...
    if start == -1:
        start = pages_text[0].find('ÉTUDE') # Add first word after title for each new doc type!!!!!
...
def extract_methods(pages_text):
    # Preprocess index page to remove all text before "1." so that title containing ":" is not a problem!!!
...
def extract_keyresults(pages_text):
    ...
    if start != -1:
        ...
        print("Neither 'KEY RESULTS' nor 'CHIFFRES CLES' was found in the text.")
...
def preprocessDataLine(data):
    ...
    result = re.sub(pattern, remove_spaces, data) # enlever les espaces au sein de ( ) ou [ ]
    return result
...
def get_table_title_description(table_text):
    ...
    description = ""
    endDesc = False # if "ADSL" in line ---> True, ADSL is the end of a table!!!!
    endValLine = 0 # Index of last line containing values
    
    for l in range(1, len(lines)):
        line = preprocessDataLine(lines[l])
        if isValueLine(line):
            endValLine = l
    
    for l in range(min(endValLine+1, len(lines)), len(lines)):
        line = preprocessDataLine(lines[l])
...
def list_page_tables(page_text):
    ...
    filtered_tables = []
    for table in tables:
        figure_index = table.find('Figure') # Des fois le texte déborde sur la figure d'après
        if figure_index != -1:
